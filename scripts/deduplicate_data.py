...
def remove_near_duplicates(df, key_columns, keep_strategy='most_complete'):
    ...
    if keep_strategy == 'most_complete':
        ...
        try:
            best_indices = df.groupby(key_columns, group_keys=False).apply(get_best_index, include_groups=False).values
        except TypeError:
            best_indices = df.groupby(key_columns, group_keys=False).apply(get_best_index).values
