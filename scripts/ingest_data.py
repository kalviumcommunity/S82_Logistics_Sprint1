...
def ingest_csv(filepath, delimiter=',', encoding='utf-8', dtype_dict=None):
    ...
    try:
        df = pd.read_csv(
            filepath, delimiter=delimiter, encoding=encoding, dtype=dtype_dict
        )
...
def ingest_json(filepath, is_nested=False):
    ...
    try:
        ...
        if is_nested:
            # Flatten nested JSON: {'customer': {'name': 'Alice'}} â†’ 'customer.name': 'Alice'
            # Note: converting to dict format that json_normalize expects
            df = pd.json_normalize(df.to_dict(orient='records'))
            print("âœ“ Nested JSON flattened to tabular format")
        print(f"âœ“ JSON loaded: {filepath}")
        print(f"  Shape: {df.shape[0]} rows Ã— {df.shape[1]} columns")
        return df
    except FileNotFoundError:
        print(f"Error: File not found - {filepath}")
    ...
    for delimiter in delimiters:
        for encoding in fallback_encodings:
            try:
                ...
                print(f"âœ“ Successfully loaded with delimiter='{delimiter}', encoding='{encoding}'")
                return df
...
if __name__ == "__main__":
    ...
    csv_df = ingest_csv(
        "data/raw/customers.csv", delimiter=',', encoding='utf-8'
    )
    document_ingestion(csv_df, "customers.csv")

    # Load JSON with flattening
    json_df = ingest_json(
        "data/raw/transactions.json", is_nested=True
    )
    ...
    print("\nâœ“ All data ingested and saved to processed/")
